...
### General
This mod makes heavy use of `TerrainGrid.SetTerrain` and `GenTemperature.TryGetTemperatureForCell`. There are warnings that display the name of mods that are patching these specific methods. It is reccomended to turn off these settings if there is too much lag. Or remove the mentioned mods.

For `TerrainGrid.SetTerrain`, settings making use of this include `Ground/Water freezes` and `Show rain effects`

For `GenTemperature.TryGetTemperatureForCell`, there is a setting `Use map-wide temperature` toggle this on and cell calculations will make use of the maps temperature instead. This includes indoors.
